...
class Abstract_Classifier(ABC):
    ...
    def _prepare_data(self, balanced = True):
        self.signal_data_balanced = tools.concat_signals(self.signal_path_csv_list, balance = balanced)
        self.bkg_data_balanced = tools.concat_signals(self.bkg_path_csv_list, balance = balanced)
                
        X , Y = tools.prepare_to_train(self.signal_data_balanced,self.bkg_data_balanced, balance = True)
        self.trainPred, self.testPred, self.trainLab, self.testLab = train_test_split(X, Y, test_size=0.30)
    ...
    def save_model(self, path_to_save = os.getcwd(), file_name = None):
        ...
        joblib.dump(self.model, os.path.join(path_to_save, file_name))
